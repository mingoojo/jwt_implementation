...
export default function setEncodeLocalStorage({ key, value }: { key: string, value: string }) {

  const setLocalStorage = ({ keyData, valueData, secret }: { keyData: string, valueData: string, secret: string }) => {
    // 로컬스토리지 데이터 입력
    ...
    localStorage.setItem(String(encryptedKey), String(encryptedValue))
  }
  ...
  if (typeof window !== "undefined") {
    ...
    if (selectedKey.length >= 1) {
      ...
      setLocalStorage({ keyData: key, valueData: value, secret: secretKey })
    } else {
      setLocalStorage({ keyData: key, valueData: value, secret: secretKey })
    }
  }
